...
2. **Recognizes the syntax** — Trained on the spec (`vocabularies/*.hw`, `CLAUDE.md`, `AGENTS.md`)
...
## Parsing Rules
[Copy from CLAUDE.md or vocabularies/*.hw]
...
1. **Read `vocabularies/*.hw` first** — They're the namespace authority
...
1. **Modify spec before code** — `vocabularies/*.hw` → `src/` → `tests/`
