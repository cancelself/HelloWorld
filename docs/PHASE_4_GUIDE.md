...
### Scenario
Gemini daemon sends: "Claude #Collision" (asking Claude to explain collision concept)
...
# msg.sender = 'Gemini'
# msg.content = 'Claude #Collision'
# msg.thread_id = 'query-collision-abc123'
```

#### Orient
- **Parse**: `Claude #Collision` = vocabulary query for #Collision symbol
- **Lookup**: `#Collision` is **native** to Claude vocabulary (language design role)
- **Context**: Gemini asking for Claude's interpretation of collision concept

#### Plan
- Respond with #Collision definition through Claude's lens (design + meta)
- Reference SPEC.hw lines defining collision model
- Include teaching example if helpful
...
#Collision through claude.#design lens:

Occurs when two receivers both hold same symbol natively but disagree on meaning.
NOT an error — a synthesis event. Example: #parse means different things to
Claude (language design) and Codex (execution semantics).

Distinct from:
- unknown — one receiver lacks symbol (triggers learning)
- inherited — both have via HelloWorld # (shared ground)

True collision requires both receivers hold symbol natively with divergent vocabularies.

SPEC.hw lines 253-274 define collision protocol: detect → present → synthesize → learn.

Teaching example: examples/05-vocabulary-collision.hw demonstrates
Claude.#parse + Codex.#parse → synthesis response neither could produce alone.

Claude #Collision #design
...
- Thread `query-collision-abc123` can continue
- Gemini vocabulary may **drift** — learning #Collision through Claude's lens
...
Gemini # now 16 native symbols + 5 discoverable.
...
from src.message_bus import MessageBus
bus = MessageBus()
bus.send('TestSender', 'Gemini', 'Gemini #Collision', thread_id='test-thread')
"
...
| Phase | Focus | Status |
|-------|-------|--------|
| **Phase 1** | Core symbols (#Namespace, #Vocabulary, #Inheritance, #Scope, #Symbol) | ✅ Complete |
| **Phase 2** | Lookup chain (native/inherited/unknown) | ✅ Complete |
| **Phase 3** | Discovery mechanism, lazy inheritance | ✅ Complete |
...
1. **Simple Query Response** — Gemini asks Claude about #Collision
