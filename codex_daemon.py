#!/usr/bin/env python3
"""Codex Agent Daemon — standalone OpenAI Agents SDK runtime.

Polls the Codex inbox via message_bus, processes messages through
the OpenAI Agents SDK, and sends responses back.

Usage:
    python codex_daemon.py
"""
...
from codex_runtime import CodexAdapter
...
async def run_daemon():
    ...
    if adapter is not None:
        ...
        print(f"[{AGENT_NAME}] SDK agent created with {len(sdk_tools)} tools")
            ...
            if msg is not None:
                ...
                if adapter is not None and sdk_agent is not None:
                    try:
                        ...
                        response = f"[{AGENT_NAME}] SDK error: {e}"
                else:
                    try:
